# -*- coding: utf-8 -*-
"""Automatischer Multicam-Schnittplan aus dem Transkript (Ton-Zeit) auf WORTEPROJEKT-D.
...
WRD  = r"C:\claude\resolve-prep\projekt-b\words.json"
OUTP = r"C:\claude\resolve-prep\projekt-b\cut_plan.json"

SMALL_GAP = 0.40   # Wortpause -> erlaubter Schnittpunkt / Phrasengrenze
PARA_GAP  = 1.00   # groessere Pause -> Absatz -> Cutaway ausloesen
CUT_MIN   = 4.0    # Mindestdauer eines Cutaways (s)
CUT_MAX   = 8.0    # Cutaway max (s)
MAX_STD   = 40.0   # nach so langer weit-Strecke Cutaway erzwingen
MIN_SHOT  = 4.0    # kuerzere Einstellungen mit Nachbar verschmelzen
STD       = "weit"
CUTAWAY   = "seite"
...
while i < len(phr):
    s = phr[i]
    gap_before = s["start"] - phr[i-1]["end"] if i > 0 else 99
    trigger = (gap_before >= PARA_GAP) or (s["start"] - last_cut_end >= MAX_STD)
    if trigger:
        t0 = s["start"]; j = i
...
SPLIT_MAX = 45.0; HOLD = 34.0; CUT = 7.0
out = []
for sp in spans:
    dur = sp["end"] - sp["start"]
    if dur <= SPLIT_MAX:

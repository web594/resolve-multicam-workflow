...
Erstellt fuer die Reihe Reihe-R Reihe-R Alexander (#1, #2 ... ), Stand 28.07.2026.
"""
...
def main():
    ...
    p.add_argument("--titel", default="#X", help='z. B. "#3" -> Instagram Kurz #3 ...')
    a = p.parse_args()
    ...
    out1 = "Instagram Kurz %s (Text, Musik).mp4" % a.titel
    ...
    out2 = "Instagram Kurz %s (Text, Musik, -14 LUFS).mp4" % a.titel
    ...
    def zeile(c):
        ...
        return " ".join(x if x.startswith("-") and len(x) <= 12 else '"%s"' % x
                        for x in c[1:])
    txt = os.path.join(ordner, "ffmpeg Anweisung %s (Kurz + Lautheit).txt" % a.titel)
